...
void TrajectoryPlanner2D::SampleShootingPoints() {
    ...
    for (int i = 0; i < num_target_; i++) {
        ...
        for (int j = 0; j < num_thread_; j++) {
            worker_thread.emplace_back(&TrajectoryPlanner2D::SampleShootingPointsSubProcess, this, i, num_chunk,
...
void TrajectoryPlanner2D::ComputePrimitives() {
    ...
    for (int i = 0; i < num_thread_; i++) {
        worker_thread.emplace_back(&TrajectoryPlanner2D::ComputePrimitivesSubProcess, this, num_chunk * (i),
                                   num_chunk * (i + 1), std::ref(primitive_list_temp[i]));
    }
    for (int i = 0; i < num_thread_; i++) {
...
void TrajectoryPlanner2D::ComputePrimitivesSubProcess(const int &start_idx, const int &end_idx,
    ...
    for (int i = start_idx; i < end_idx; i++) {
        { // x-component
            ...
            bernstein_coeff_temp[2] = drone_state_.px + 0.4f * planning_horizon_ * drone_state_.vx +
                                      0.05f * planning_horizon_square * drone_state_.ax;
            bernstein_coeff_temp[3] = 0.16666667f * shooting_points_[i].x + 0.83333333f * drone_state_.px +
                                      0.43333333f * planning_horizon_ * drone_state_.vx +
                                      0.06666667f * planning_horizon_square * drone_state_.ax;
            bernstein_coeff_temp[4] =
                    0.5f * shooting_points_[i].x + 0.5f * drone_state_.px + 0.3f * planning_horizon_ * drone_state_.vx +
                    0.05f * planning_horizon_square * drone_state_.ax;
        ...
        { // y-component
            ...
            bernstein_coeff_temp[2] = drone_state_.py + 0.4f * planning_horizon_ * drone_state_.vy +
                                      0.05f * planning_horizon_square * drone_state_.ay;
            bernstein_coeff_temp[3] = 0.16666667f * shooting_points_[i].y + 0.83333333f * drone_state_.py +
                                      0.43333333f * planning_horizon_ * drone_state_.vy +
                                      0.06666667f * planning_horizon_square * drone_state_.ay;
            bernstein_coeff_temp[4] =
                    0.5f * shooting_points_[i].y + 0.5f * drone_state_.py + 0.3f * planning_horizon_ * drone_state_.vy +
                    0.05f * planning_horizon_square * drone_state_.ay;
...
void TrajectoryPlanner3D::SampleShootingPoints() {
    ...
    for (int i = 0; i < num_target_; i++) {
        ...
        vector<vector<Point>> shooting_point_temp(num_thread_);
        for (int j = 0; j < num_thread_; j++) {
            worker_thread.emplace_back(&TrajectoryPlanner3D::SampleShootingPointsSubProcess, this, i, num_chunk,
                                       std::ref(shooting_point_temp[j]));
        }
        for (int j = 0; j < num_thread_; j++) {
            worker_thread[j].join();
...
void TrajectoryPlanner3D::ComputePrimitives() {
    ...
    PrimitiveListSet primitive_list_temp;
    for (int i = 0; i < num_thread_; i++) {
        worker_thread.emplace_back(&TrajectoryPlanner3D::ComputePrimitivesSubProcess, this, num_chunk * (i),
                                   num_chunk * (i + 1), std::ref(primitive_list_temp[i]));
    }
    for (int i = 0; i < num_thread_; i++) {
        worker_thread[i].join();
...
void TrajectoryPlanner3D::ComputePrimitivesSubProcess(const int &start_idx, const int &end_idx,
    ...
    for (int i = start_idx; i < end_idx; i++) {
        ...
        { // z-component
            ...
            bernstein_coeff_temp[3] = 0.16666667f * shooting_points_[i].z + 0.83333333f * drone_state_.pz +
                                      0.43333333f * planning_horizon_ * drone_state_.vz +
                                      0.06666667f * planning_horizon_square * drone_state_.az;
            bernstein_coeff_temp[4] =
                    0.5f * shooting_points_[i].z + 0.5f * drone_state_.pz + 0.3f * planning_horizon_ * drone_state_.vz +
                    0.05f * planning_horizon_square * drone_state_.az;
