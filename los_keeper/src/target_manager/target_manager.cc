...
los_keeper::TargetManager::TargetManager(const los_keeper::PredictionParam &param) : param_(param) {
}
...
void los_keeper::TargetManager2D::SampleEndPoints() {
    ...
    for (int i = 0; i < target_state_list_.size(); i++) {
        ...
        vector<vector<Point>> end_point_temp(param_.num_thread);
...
void los_keeper::TargetManager2D::ComputePrimitives() {
    ...
    for (int i = 0; i < num_target_; i++) {
        int num_chunk = param_.num_sample / param_.num_thread;
        vector<thread> worker_thread;
        PrimitiveListSet primitive_list_temp(param_.num_thread);
        for (int j = 0; j < param_.num_thread; j++) {
        ...
        for (int j = 0; j < structured_obstacle_poly_list_.size(); j++) {
            is_close =
                    ...
                    powf(target_state_list_[i].py -
                         ...
                         2) <
                    param_.detect_range * param_.detect_range;
            if (is_close)
...
bool los_keeper::TargetManager2D::CheckCollision() {
    ...
    if (is_cloud_empty and is_structured_obstacle_empty) { // Case I: No Obstacle
        for (int i = 0; i < num_target_; i++) {
            ...
            for (int j = 0; j < param_.num_sample; j++) {
...
void los_keeper::TargetManager2D::CalculateCentroid() {
    ...
    for (int i = 0; i < num_target_; i++) {
        int num_chunk = (int) primitive_safe_total_index_[i].size() / param_.num_thread;
        vector<thread> worker_thread;
        vector<std::pair<int, float>> min_dist_pair_temp(param_.num_thread);
        for (int j = 0; j < param_.num_thread; j++) {
...
void los_keeper::TargetManager2D::CheckStructuredObstacleCollision() {
    ...
    for (int i = 0; i < num_target_; i++) {
        int num_chunk = (int) primitives_list_[i].size() / param_.num_thread;
        vector<thread> worker_thread;
        IndexListSet primitive_safe_structured_obstacle_index_temp(param_.num_thread);
        for (int j = 0; j < param_.num_thread; j++) {
...
std::vector<LinearConstraint2D> los_keeper::TargetManager2D::GenLinearConstraint() {
    ...
    for (int i = 0; i < num_target_; i++) {
        ...
        decomp_util.set_local_bbox(
                Vec2f(0.5f * param_.virtual_pcl_zone_width, 0.5f * param_.virtual_pcl_zone_width));
...
void los_keeper::TargetManager2D::CalculateSafePclIndex(
    ...
    for (int i = 0; i < num_target_; i++) {
        int num_chunk = param_.num_sample / param_.num_thread;
        vector<thread> worker_thread;
        IndexListSet safe_pcl_index_temp(param_.num_thread);
        for (int j = 0; j < param_.num_thread; j++) {
...
void los_keeper::TargetManager2D::SampleEndPointsSubProcess(const int &target_id,
    ...
    Point end_point_center{float(target_state_list_[target_id].px +
                                 target_state_list_[target_id].vx * param_.planning_horizon),
                           float(target_state_list_[target_id].py +
                                 target_state_list_[target_id].vy * param_.planning_horizon),
                           float(target_state_list_[target_id].pz +
                                 target_state_list_[target_id].vz * param_.planning_horizon)};
    uint n_cols = 2;
    uint n_rows = param_.num_sample;
    ...
    covar << (0.5f * 0.33333333f * param_.acc_max * param_.planning_horizon * param_.planning_horizon), 0, 0,
            (0.5f * 0.33333333f * param_.acc_max * param_.planning_horizon * param_.planning_horizon);
    ...
    for (int j = start_idx; j < end_idx; j++) {
        { // x-coefficient
            ...
            bernstein_coeff_temp[1] = target_state_list_[target_id].px +
                                      0.33333333f * target_state_list_[target_id].vx * param_.planning_horizon;
            bernstein_coeff_temp[2] = 0.5f * target_state_list_[target_id].px +
                                      0.5f * end_points_[target_id][j].x +
                                      0.16666667f * target_state_list_[target_id].vx * param_.planning_horizon;
        ...
        { // y-coefficient
            ...
            bernstein_coeff_temp[1] = target_state_list_[target_id].py +
                                      0.33333333f * target_state_list_[target_id].vy * param_.planning_horizon;
            bernstein_coeff_temp[2] = 0.5f * target_state_list_[target_id].py +
                                      0.5f * end_points_[target_id][j].y +
                                      0.16666667f * target_state_list_[target_id].vy * param_.planning_horizon;
        ...
        { // z-coefficient
            ...
            bernstein_coeff_temp[1] = target_state_list_[target_id].pz +
                                      0.33333333f * target_state_list_[target_id].vz * param_.planning_horizon;
            bernstein_coeff_temp[2] = target_state_list_[target_id].pz +
                                      0.66666667f * target_state_list_[target_id].vz * param_.planning_horizon;
            bernstein_coeff_temp[3] =
                    target_state_list_[target_id].pz + target_state_list_[target_id].vz * param_.planning_horizon;
...
los_keeper::TargetManager2D::TargetManager2D() {

}

los_keeper::TargetManager2D::TargetManager2D(const los_keeper::PredictionParam &param) : TargetManager(param) {
}
...
std::vector<LinearConstraint3D> los_keeper::TargetManager3D::GenLinearConstraint() {
    ...
    for (int i = 0; i < num_target_; i++) {
        ...
        decomp_util.set_obs(obstacle_pcl);
        decomp_util.set_local_bbox(Vec3f(0.5f * param_.virtual_pcl_zone_width, 0.5f * param_.virtual_pcl_zone_width,
                                         0.5f * param_.virtual_pcl_zone_height));
        decomp_util.dilate(0.1);
        polys.push_back(decomp_util.get_polyhedron());
        auto poly_hedrons = decomp_util.get_polyhedron();
...
void los_keeper::TargetManager3D::SampleEndPointsSubProcess(const int &target_id,
    ...
    covar << (0.5f * 0.33333333f * param_.acc_max * param_.planning_horizon * param_.planning_horizon), 0, 0, 0,
            (0.5f * 0.33333333f * param_.acc_max * param_.planning_horizon * param_.planning_horizon), 0, 0, 0,
            (0.5f * 0.33333333f * param_.acc_max * param_.planning_horizon * param_.planning_horizon);
    Eigen::EigenMultivariateNormal<float> normX_solver1(mean, covar);
    ...
    for (int j = start_idx; j < end_idx; j++) {
        ...
        { // z-coefficient
            ...
            bernstein_coeff_temp[2] = 0.5f * target_state_list_[target_id].pz +
                                      ...
                                      0.16666667f * target_state_list_[target_id].vz * param_.planning_horizon;
...
los_keeper::TargetManager3D::TargetManager3D() {
}

los_keeper::TargetManager3D::TargetManager3D(const los_keeper::PredictionParam &param) : TargetManager(param) {
}
